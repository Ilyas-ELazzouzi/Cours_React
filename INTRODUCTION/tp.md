#  Exercice – Explication de code et utilisation des Hooks

## Partie 1 – Compréhension du code

Voici un code que vous devez **analyser et expliquer**, en vous appuyant sur le cours et la documentation de React.

---

```html
<!DOCTYPE html>
<html lang="fr">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>React – useEffect & useState</title>

  <!-- React, ReactDOM et Babel -->
  <script src="https://unpkg.com/react@18/umd/react.development.js" crossorigin></script>
  <script src="https://unpkg.com/react-dom@18/umd/react-dom.development.js" crossorigin></script>
  <script src="https://unpkg.com/@babel/standalone/babel.min.js"></script>

  <!-- TailwindCSS -->
  <script src="https://cdn.tailwindcss.com"></script>
</head>

<body class="bg-slate-900 text-white min-h-screen flex flex-col items-center justify-center p-4">
  <h1 class="text-3xl font-bold mb-6">Suivi de température</h1>
  <div id="root" class="w-full max-w-xl"></div>

  <script type="text/babel">
    const { useState, useEffect } = React;

    function Counter() {
      const [count, setCount] = useState(0);

      useEffect(() => {
        console.log("MONTAGE / MISE À JOUR :", count);

        // Fonction de nettoyage (appelée au démontage)
        return () => {
          console.log("NETTOYAGE :", count);
        };
      });

      return (
        <React.Fragment>
          <p>Compteur : {count}</p>
          <button
            onClick={() => setCount(count + 1)}
            className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded mt-2"
          >
            +1
          </button>
        </React.Fragment>
      );
    }

    function App() {
      const [status, setStatus] = useState(true);

      return (
        <div className="bg-slate-800 p-4 rounded-xl shadow-lg">
          <button
            onClick={() => setStatus(!status)}
            className="bg-gray-600 hover:bg-gray-700 px-4 py-2 rounded mb-4"
          >
            Changer le statut
          </button>

          {status && <Counter />}
        </div>
      );
    }

    ReactDOM.createRoot(document.getElementById("root")).render(<App />);
  </script>
</body>
</html>
```

---

### ✏️ Travail demandé

1. **Expliquez le fonctionnement du code** :

   * Quel est le rôle de `useState` dans `Counter` et `App` ?
  Dans count : crée une variable count qui contient la valeur actuelle du compteur, et une fonction setCount pour la modifier
  Valeur initiale de count est 0 et à chaque clic sur le bouton +1 la fonction setCount(count + 1) met à jour la valeur de countce qui provoque un nouveau rendu du composant
  Dans App : status est un booléen qui détermine si le composant Counter doit être affiché ou non

   * À quoi sert `useEffect` ici ?
  Il affiche des messages dans la console pour montrer quand le composant Counter est monté, mis à jour ou démonté

   * Quand la fonction de nettoyage est-elle appelée ?
  Avant le prochain appel de useEffect et quand l'élément est retiré du DOM (démonté)

   * Que se passe-t-il quand on clique sur "Changer le statut" ?
  Si status était true il devient false donc Counter n’est plus affiché et l'inverse si au départ il était false

   * Quelle est la différence entre le **montage**, la **mise à jour**, et le **démontage** du composant `Counter` ?
   Montage : le composant est créé et affiché pour la première fois dans le DOM.
  Mise à jour : le composant est déjà présent, mais un de ses états (ici count) change.

  Démontage : le composant est supprimé du DOM (par exemple quand status devient false).

2. Appuyez-vous sur les concepts vus en cours :

   * Le cycle de vie d'un composant fonctionnel.
  Montage : useEffect s’exécute après le premier affichage
  Mise à jour : useEffect s’exécute à chaque changement d’état ou de props
  Démontage : la fonction de nettoyage à l’intérieur de useEffect s’exécute avant que le composant soit retiré

   * Le comportement de `useEffect` à chaque rendu.
  le useEffect est appelé après chaque rendu donc à chaque mise à jour de l’état

---

## Partie 2 – Exemple d'utilisation de `useReducer`

Donnez un **exemple simple** d'un composant React utilisant le hook `useReducer` pour gérer un compteur, `useReducer` est un super `useState`
Vous pouvez vous inspirer de ce modèle :

```jsx
import React, { useReducer } from "react";

function reducer(state, action) {
  switch (action.type) {
    // TODO 
    default:
      return state;
  }
}

export default function CounterReducer() {

  const [state, dispatch] = useReducer(reducer, { count: 0 });

  return (
    <div className="p-4 bg-slate-800 rounded-lg text-white">
    
    </div>
  );
}
```

